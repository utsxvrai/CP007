...
vi pfactors(int n) { vi v; while (n % 2 == 0) { v.pb(2); n /= 2; } for (int i = 3; i <= sqrt(n); i += 2) { while (n % i == 0) { v.pb(i); n /= i; } } if (n > 2) v.pb(n); return v; } // vector of prime factors   factors(n)
...
// write an optimized code to find the factors of a number
...
void C_R_7()
{
    ...
    auto it = lower_bound(fac.begin(),fac.end(),k);
    int s= *it;
    if(s==k){
        cout<<n/s<<endl;
        return;
