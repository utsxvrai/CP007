...
template<typename T> int sumvec(vector<T>v){int n=v.size();int s=0;for(int i= 0 ; i < n ; i++)s+=v[i];return s;}
...
void C_R_7()
{
    ...
    int lb1=lower_bound(pre.begin(), pre.end(),(k+1)/2)-pre.begin();
    int lb2=lower_bound(suff.begin(), suff.end(), k/2)-suff.begin();
    reverse(suff.begin(),suff.end());
    lb2=n-lb2-1;
    if(lb1>lb2){
        cout<<n<<"\n";
