...
void C_R_7()
{
    ...
    for(int i = 1 ; i < n-1 ; i++){
        char z= s[i];
        char x = s[i+1];
        if((z == 'b'|| z == 'c'|| z =='d') && (x == 'a'|| x == 'e')){
            v.pb(s.substr(start,i-start));
            start = i;
        }
        else if((z == 'b'|| z == 'c'|| z =='d') && (x == 'b'|| x == 'c'|| x =='d')){
            v.pb(s.substr(start,i-start+1));
            start = i+1;
        }
    }
    
    v.pb(s.substr(start,n-start));
    // i wanted to remove white spacers from the vector
    vector<string> v1;
