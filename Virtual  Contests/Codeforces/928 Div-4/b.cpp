...
int solve (vector<vector<int>> &v, int n, int m, int i, int j)
{
    ...
    return max(solve(v,n,m,i+1,j) , solve(v,n,m,i,j+1)) + v[i][j];
...
void C_R_7()
{
    ...
    vector<int> temp(n,0); // Initialize temp vector
    ...
    // if (tcheck) cout << "Triangle" << endl;
    // else if(scheck) cout << "Square" << endl;
