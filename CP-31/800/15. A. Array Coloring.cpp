...
vi factors(int n)
{
    ...
    if (n > 2)
        v.pb(n);
    return v;
} // vector of prime factors   factors(n)
unsigned int power(int x, int y, int p)
{
    unsigned int s = 1;
    x = x % p;
    while (y > 0)
...
vi prePrime(int n)
{
    ...
    return v;
} // vector of prime numbers less than n  prePrime(n
